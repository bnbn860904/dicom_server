...
class U_Net():    
    def __init__(self):
        ...
        # u_net
        self.unet = self.build_unet()  # 創建網絡變量        
        #self.unet.summary()
    ...
    def train(self, epochs=100, batch_size=8): 
        ...
        if not osp.exists('./weights/'):
            os.mkdir('./weights/')
        # 獲得數據
        train_img, train_lbl = self.load_data()
        
        np.random.seed(116)  # 設置相同的隨機種子，確保數據匹配
        ...
        # 設置訓練的checkpoint
        filepath = './weights/liver' + '-{epoch:02d}-{val_metric_fun:.3f}.h5'
        ...
        self.unet.compile(loss='binary_crossentropy', optimizer=self.optimizer, metrics=[self.metric_fun])
        # 進行訓練
        results = self.unet.fit(train_img, train_lbl, batch_size=batch_size, epochs=epochs, verbose=1,
